  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errMsg = data?.error || data?.message || data?.detalle || 'No se pudo enviar email';
        showToast(errMsg);
      }
    } catch (err) {
  ...
  return (
    <View style={styles.flex}>
      ...
      >
        ...
        <Text style={[styles.title, dynamicStyles.title]}>Coloca tu correo electrónico</Text>
        ...
        <TouchableOpacity
          ...
          disabled={loading}
        >
          {loading ? <ActivityIndicator color="#0046ff" /> : <Text style={[styles.buttonText, dynamicStyles.buttonText]}>Enviar</Text>}
        </TouchableOpacity>
